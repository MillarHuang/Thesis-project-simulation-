...
L[,1]=rnorm(n,0+U,1)
###########################################################
#positivity violation:if L_t >= tau, forced to receive treatment
###########################################################
#A0 for n subjects
A[,1]=ifelse(L[,1]>=tau,1, rbinom(n,1,expit(-2+0.5*L[,1])))
#A1,...,AT for n subjects
for(k in 2:n.visit){
  L[,k]=rnorm(n,0.8*L[,k-1]-A[,k-1]+0.1*(k-1)+U,1)
  A[,k]=ifelse(L[,k]>=tau, 1, rbinom(n,1,expit(-2+0.5*L[,k]+A[,k-1])))
}
